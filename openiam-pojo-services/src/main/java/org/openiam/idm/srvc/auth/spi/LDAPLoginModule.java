...
public class LDAPLoginModule extends AbstractLoginModule {
	...
	public Subject login(AuthenticationContext authContext) throws AuthenticationException {
        ...
        if (ne == null) {
            log("AUTHENTICATION", "AUTHENTICATION", "FAIL", "INVALID LOGIN", domainId, null, principal, null, null, clientIP, nodeIP);
        ...
        if (lg == null) {
         log("AUTHENTICATION", "AUTHENTICATION", "FAIL", "MATCHING IDENTITY NOT FOUND", domainId, null, principal, null, null, clientIP, nodeIP);
          throw new AuthenticationException(AuthenticationConstants.RESULT_INVALID_LOGIN);
        }
        ...
        user = this.userManager.getUserWithDependent(lg.getUserId(),false);
		 ...
		 if (tempCtx == null) {
			 log("AUTHENTICATION", "AUTHENTICATION", "FAIL", "RESULT_INVALID_PASSWORD", domainId, null, principal, null, null, clientIP, nodeIP);
...
/*		if (user != null && user.getStatus() != null ) {
	...
	*/
        ...
        if (user.getStatus() != null) {
            if (user.getStatus().equals(UserStatusEnum.PENDING_START_DATE)) {
                if (!pendingInitialStartDateCheck(user, curDate)) {
                    log("AUTHENTICATION", "AUTHENTICATION", "FAIL", "INVALID USER STATUS", domainId, null, principal, null, null, clientIP, nodeIP);
                    throw new AuthenticationException(AuthenticationConstants.RESULT_INVALID_USER_STATUS);
                }
            }
            if (!user.getStatus().equals(UserStatusEnum.ACTIVE) && !user.getStatus().equals(UserStatusEnum.PENDING_INITIAL_LOGIN)) {
		...
		log("AUTHENTICATION", "AUTHENTICATION", "SUCCESS", null, domainId, user.getUserId(), distinguishedName, null, null, clientIP, nodeIP);
